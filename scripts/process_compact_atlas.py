"""Build compact host-centered relational atlas data with higher parasite taxonomy.

Usage:
    python process_compact_atlas.py parasite_export.csv data

Expected columns:
GUID, SCIENTIFIC_NAME, VERBATIM_DATE, DEC_LAT, DEC_LONG,
KINGDOM, PHYLUM, PHYLCLASS, PHYLORDER, FAMILY, GENUS, SPECIES,
    ...
    for _,r in df.iterrows():
        ...
        if pg not in parasite_guid_to_id:
            ...
            parasite_meta.append([
                ...
                sid(None if pd.isna(r.get("KINGDOM")) else r.get("KINGDOM")),
                sid(None if pd.isna(r.get("PHYLUM")) else r.get("PHYLUM")),
                sid(None if pd.isna(r.get("PHYLCLASS")) else r.get("PHYLCLASS")),
                sid(None if pd.isna(r.get("PHYLORDER")) else r.get("PHYLORDER")),
                sid(None if pd.isna(r.get("FAMILY")) else r.get("FAMILY")),
